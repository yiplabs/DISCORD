...
- `DISCORD_BOT_XP_SECRET` — the same server-to-server secret configured on
  the DVC API.
- Every detected live stream from either watched creator pings `@everyone`.
  Regular uploads never ping `@everyone`.
...
has durable state and an audit trail. Live YouTube announcements always
mention `@everyone`; regular uploads never do. An optional
`YOUTUBE_MENTION_ROLE_ID` can notify an opt-in role for uploads without changing
the live-stream policy.
